...
async function main() {
    try {
        ...
        if (args.values['add-server']) {
            ...
            if (!serverCommand) {
                console.error('Error: --server-command is required when adding a server');
                process.exit(1);
            }
        ...
        if (!serverCommand) {
            ...
            console.error('Use --server <name> to use a configured server, or --server-command to specify a server directly.');
            console.error('Use --list-servers to see available servers.');
            process.exit(1);
        }
